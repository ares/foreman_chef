module ForemanChef
  class CachedRunList < ActiveRecord::Base
    ...
    # on save (gets triggered only when there's attr change) we should enforce the change on chef server
    # we should check list.changed? just to be sure, it works even with serialized list
    # opposite direction should work differently, on existing host form we should check chef-server version and
    #   display warning if cache is out of date and have action to refresh it
    #   or we could always load server version
    # for new hosts that shouldn't matter at all
    # we could also start updating cache based on incoming facts
    # later - roles should become first class citizen and be synced together with environments
    ...
    def self.parse_array(run_list_array)
      ...
      elsif run_list_array.first.is_a?(String)
        run_list_array
      else
